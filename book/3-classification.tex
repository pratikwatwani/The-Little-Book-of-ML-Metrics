...
\begin{center}
    \tikz{
        ...
        \draw[-latex,nmlcyan, semithick] ($(a.north)+(0.5,-0.1)$) to[bend right=15] node[pos=1, left] {true positives} +(-1,.5);
        \draw[-latex,cyan, semithick] ($(a.north)+(3.5,-0.1)$) to[bend left=15] node[pos=1, right] {true negatives} +(1,.5);
        \draw[-latex,nmlpurple, semithick] ($(a.south)+(1.5,0.3)$) to[bend left=15] node[pos=1, left] {misclassifications (FN, FP)} +(-1,-.5);
    }
\end{center}
...
the total number of instances predicted as positive (true positives + false positives). It answers the question: “Out of all the cases indicated as positive, how many are actual positives?”
...
It is an extension of the F1-score, where the parameter $\beta$ determines the weight of recall relative to precision.
...
The Receiver Operating Characteristic (ROC) plot illustrates a classifier's performance at different thresholds. The curve helps us understand the trade-off between FPR
...
\begin{center}
    \tikz{
        ...
        \draw[-latex,nmlcyan, semithick] ($(a.north)+(1.0,-0.1)$) to[bend left=15] node[pos=1, right] {log loss of the model} +(1,.5);
        \draw[-latex,nmlpurple, semithick] ($(a.south)+(1.0,0.1)$) to[bend left=15] node[pos=1, left] {log loss of the null model} +(-1,-.5);
    }
\end{center}
...
\begin{center}
    \tikz{
        ...
        \draw[-latex,nmlcyan, semithick] ($(a.north)+(0.2,-0.1)$) to[bend right=15] node[pos=1, left] {true positives} +(-1,.5);
        \draw[-latex,cyan, semithick] ($(a.north)+(1.3,-0.1)$) to[bend left=15] node[pos=1, right] {true negatives} +(1,.5);
        \draw[-latex,nmlpurple, semithick] ($(a.south)+(3.0,0.3)$) to[bend left=15] node[pos=1, left] {misclassifications (FP, FN)} +(-1,-.5);
    }
\end{center}
...
\begin{center}
    \tikz{
        ...
        \draw[-latex,nmlcyan, semithick] ($(a.north)+(0.5,-0.1)$) to[bend left=15] node[pos=1, right] {observed agreement} +(1,.5);
        \draw[-latex,nmlpurple, semithick] ($(a.south)+(0.8,0.2)$) to[bend left=15] node[pos=1, left] {expected agreement by chance} +(-1,-.5);
    }
\end{center}
...
The Matthews Correlation Coefficient, also known as Phi coefficient for binary classification, is a measure of association between predicted and actual classifications
...
\begin{center}
    \tikz{
        ...
        \draw[-latex,nmlcyan, semithick] ($(a.north)+(-0.2,-0.1)$) to[bend right=15] node[pos=1, left] {true positives} +(-1,.5);
        \draw[-latex,cyan, semithick] ($(a.north)+(1.2,-0.1)$) to[bend left=15] node[pos=1, right] {true negatives} +(1,.5);
        \draw[-latex,nmlpurple, semithick] ($(a.south)+(0.5,0.3)$) to[bend left=15] node[pos=1, left] {misclassifications (FP, FN)} +(-1,-.5);
    }
\end{center}
...
\textbf{Figure. MCC} as a function of TP and FP, with FN=20 and TN=80 fixed. MCC increases as true positives grow and decreases as
false positives accumulate. Notice MCC can become negative when the classifier performs worse than random.

\orangebox{Did you know that...}
{MCC was introduced by biochemist Brian W. Matthews in 1975 for assessing the quality of protein secondary structure predictions. It has since become one of the most
recommended metrics in machine learning, with several studies arguing it should replace accuracy and F1-score as the standard evaluation metric for binary classification.}

\textbf{Other related metrics}

Other metrics commonly explored alongside MCC are the F1-score, Cohen's Kappa, and Balanced Accuracy. Unlike F1, MCC takes into account all four confusion matrix categories and is
generally regarded as a more informative single score for binary classification evaluation.
