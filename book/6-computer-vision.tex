...
A Pixel Accuracy of 1.0 indicates perfect segmentation in the case of a balanced dataset, while lower values suggest a higher proportion of misclassified pixels.
...
\textbf{Figure.} When background dominates (85\%), Pixel Accuracy can be misleadingly high (90\%) even if minority classes are poorly segmented. Mean Pixel Accuracy (55\%) reveals the true performance.
...
\textbf{Figure.} OKS applies a Gaussian falloff to localization errors. Strict keypoints (eyes, k=0.05) require precise predictions, while lenient keypoints (hips, k=0.20) tolerate larger errors.
...
\begin{center}
    \tikz{
        ...
        \draw[overlay,-latex,nmlred, semithick] ($(a.north)+(2.1,-0)$) to[bend right=15] node[pos=1, left] {maximum possible pixel}
        +(-1,.5); 
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.south)+(2,-0.1)$) to[bend left=15] node[pos=1, left] {mean squared error}
        +(-1,-.5);
    }
\end{center}
...
\begin{center}
    \tikz{
        ...
        \draw[overlay,-latex,nmlred, semithick] ($(a.south)+(1,0.5)$) to[bend right=15] node[pos=1, right] {original pixel value}
        +(1,-.5);
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.north)+(2.3,-0.4)$) to[bend right=15] node[pos=1, left] {reconstructed pixel
        value} +(-1,.5);    
    }
\end{center}
...
{
\item It does not always align with human perception of image quality.
...
or categorical sets. It is especially popular in image segmentation, information retrieval and
...
\textbf{Figure.} Dice coefficient for three levels of overlap between predicted (purple) and ground truth (cyan/red) segmentation masks. The intersection (cyan) drives the score.
...
\textbf{Figure.} PQ decomposition into Segmentation Quality (SQ) and Recognition Quality (RQ). Model B has good segmentation but poor detection; Model C is the opposite. PQ captures both aspects.
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/SSIM_vs_PSNR.png}
\end{figure*}

\textbf{Figure.} SSIM vs PSNR (normalized) across distortion types. Both metrics degrade with increasing distortion, but SSIM better captures perceptually significant changes like structural damage.
