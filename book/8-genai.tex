...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/Perplexity_training.png}
\end{figure*}

\textbf{Figure.} Perplexity decreases during training as the model learns to predict text better. It starts high (uncertain) and converges to a low value.
...
For semantic similarity beyond surface-level prediction, see BERTScore and MAUVE. For evaluating generated text
quality with human judgments, see MOS.
...
\begin{center}
    \tikz{
        \node[inner sep=2pt, font=\Large] (a) {
            {
                $\displaystyle
                F_{\text{BERT}} = 2 \cdot \frac{{\color{nmlcyan}P_{\text{BERT}}} \cdot {\color{nmlpurple}R_{\text{BERT}}}}{{\color{nmlcyan}P_{\text{BERT}}} + {\color{nmlpurple}R_{\text{BERT}}}}
                $
            }
        };
        \draw[overlay,-latex,nmlcyan, semithick] ($(a.north)+(0.5,-0.3)$) to[bend right=15] node[pos=1, left] {embedding precision} +(-1,.5);
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.north)+(2.5,-0.3)$) to[bend left=15] node[pos=1, right] {embedding recall} +(1,.5);
    }
\end{center}
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/BERTScore_vs_BLEU.png}
\end{figure*}

\textbf{Figure.} BERTScore vs BLEU across translation scenarios. BERTScore rewards synonyms and paraphrases that BLEU misses, while both detect wrong meanings.
...
BERTScore provides token-level semantic similarity, complementing surface-level metrics like BLEU and ROUGE (see NLP chapter).
For distributional comparison of generated text, see MAUVE.
...
\begin{center}
    \tikz{
        \node[inner sep=2pt, font=\Large] (a) {
            {
                $\displaystyle
                \text{MAUVE} = \exp\left(-{\color{nmlcyan}\text{KL}}({\color{nmlpurple}Q} \| {\color{nmlred}R_\lambda})\right)
                $
            }
        };
        \draw[overlay,-latex,nmlpurple, semithick] ($(a.north)+(1.5,-0.1)$) to[bend right=15] node[pos=1, left] {model distribution} +(-1,.5);
        \draw[overlay,-latex,nmlred, semithick] ($(a.north)+(2.5,-0.1)$) to[bend left=15] node[pos=1, right] {human-model mixture} +(1,.5);
        \draw[overlay,-latex,nmlcyan, semithick] ($(a.south)+(-0.5,0.3)$) to[bend left=15] node[pos=1, left] {KL divergence} +(-1,-.5);
    }
\end{center}
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/MAUVE_frontier.png}
\end{figure*}

\textbf{Figure.} MAUVE frontier curves. A good model (blue, MAUVE~0.92) produces text distributions close to human text. A poor model (red, MAUVE~0.65) diverges more. The area under the curve is the MAUVE score.
...
MAUVE evaluates distributional similarity, complementing BERTScore (token-level) and Perplexity (likelihood-based).
For image generation rather than text, see FID and IS.
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/IS_quality_levels.png}
\end{figure*}

\textbf{Figure.} Inception Score across different generation quality levels. Random noise scores near 1, while state-of-the-art models achieve IS values above 80.
...
IS measures quality and diversity of generated images independently. FID compares generated images against real images,
making it more reliable. For perceptual similarity, see LPIPS.
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/FID_quality_levels.png}
\end{figure*}

\textbf{Figure.} FID across quality levels (lower is better). State-of-the-art models achieve FID around 5, while random noise exceeds 250.
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/LPIPS_vs_L2.png}
\end{figure*}

\textbf{Figure.} LPIPS (perceptual) vs L2 (pixel) distance for different distortion types. LPIPS correctly ranks structure and texture changes as more perceptually different than color shifts, while L2 does not.
...
LPIPS measures perceptual similarity between two specific images. For distributional comparison across many images, use FID.
For pixel-level comparison, see PSNR and SSIM in the Computer Vision chapter.
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/CLIP_Score_scatter.png}
\end{figure*}

\textbf{Figure.} CLIP Score vs FID for different models. Good models (blue) achieve high CLIP scores (strong text-image alignment) and low FID (realistic images). Poor models (red) score low on both.
...
CLIP Score measures text-image alignment. For more fine-grained compositional evaluation, see DSG and VQAScore.
For image quality without text, use FID or IS.
...
DSG provides fine-grained compositional evaluation. For a simpler text-image alignment score, use CLIP Score.
For a VQA-based alternative, see VQAScore.
...
\begin{center}
    ...
    \tikz{
        ...
        \draw[overlay,-latex,nmlred, semithick] ($(a.north)+(3.0,-0.2)$) to[bend left=15] node[pos=1, right] {prompt question} +(1,.5);
    }
\end{center}
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/VQAScore_alignment.png}
\end{figure*}

\textbf{Figure.} VQAScore drops sharply as the image deviates from the prompt. An exact match scores 0.95, while an unrelated image scores near zero.
...
VQAScore and DSG both evaluate compositional text-image alignment. For a simpler cosine-similarity approach, see CLIP Score.
...
\begin{center}
    \tikz{
        \node[inner sep=2pt, font=\Large] (a) {
            {
                $\displaystyle
                MOS = \frac{1}{{\color{nmlred}N}} \sum_{i=1}^{N} {\color{nmlcyan}r_i}
                $
            }
        };
        \draw[overlay,-latex,nmlred, semithick] ($(a.south)+(-0.3,0.2)$) to[bend left=15] node[pos=1, left] {number of listeners} +(-1,-.5);
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=\textwidth]{figures/MOS_distributions.png}
\end{figure*}

\textbf{Figure.} Distribution of listener ratings for high-quality (left, MOS=4.7) vs low-quality (right, MOS=1.9) audio. The dashed line shows the mean rating.
...
MOS is the gold standard for subjective audio quality but is expensive to collect. For automated alternatives,
see PESQ (speech quality) and STOI (intelligibility).
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/PESQ_conditions.png}
\end{figure*}

\textbf{Figure.} PESQ scores under different noise conditions. Clean speech scores 4.5 (excellent), while heavily degraded speech drops to 1.2 (poor).
...
PESQ focuses on overall speech quality, while STOI focuses on intelligibility. Both aim to approximate MOS without requiring human listeners.
...
\begin{figure*}[ht!]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/STOI_vs_SNR.png}
\end{figure*}

\textbf{Figure.} STOI follows a sigmoid curve against signal-to-noise ratio. Intelligibility drops sharply below 0 dB SNR and saturates near 1.0 above 20 dB.
...
STOI focuses on intelligibility, while PESQ evaluates overall quality. Both are automated proxies for MOS.
For text-based evaluation of generated audio transcriptions, see WER in the NLP chapter.
...
GLUE was particularly useful for transfer learning approaches, like BERT, where knowledge gained on one task can benefit others. Its format
...
The result is a more challenging and focused benchmark that highlights the limits of current models rather than their strengths. BBH has since
...
But SQuAD 1.1 had a limitation, every question was guaranteed to have an answer in the passage. To make the benchmark more realistic, SQuAD 2.0
